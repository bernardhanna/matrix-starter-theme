...
curl_setopt($ch, CURLOPT_USERAGENT, 'matrix');
$releases = json_decode(curl_exec($ch), true);
curl_close($ch);

// Download $releases[0]['zipball_url'] with curl and save it to .matrix-setup/trellis.zip
...
$zip = curl_exec($ch);
curl_close($ch);
file_put_contents('.matrix-setup/trellis.zip', $zip);
if (curl_error($ch)) {
    echo curl_error($ch);
}

// Unzip .matrix-setup/trellis.zip to trellis/
$zip = new ZipArchive();
if ($zip->open('.matrix-setup/trellis.zip') === true) {
...
// Delete .matrix-setup/trellis.zip
unlink('.matrix-setup/trellis.zip');
...
// Overwrite `trellis/deploy-hooks/build-after.yml` with `.matrix-setup/trellis/build-after.yml`
// Overwrite `trellis/deploy-hooks/build-before.yml` with `.matrix-setup/trellis/build-before.yml`
copy('.matrix-setup/trellis/build-after.yml', 'trellis/deploy-hooks/build-after.yml');
copy('.matrix-setup/trellis/build-before.yml', 'trellis/deploy-hooks/build-before.yml');
